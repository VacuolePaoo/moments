...
export function MomentsHome() {
  ...
  useEffect(() => {
    const scrollKey = targetDate ? `${targetDate}:${isAdmin ? "admin" : "visitor"}` : null
    if (!targetDate || posts.length === 0 || scrollCompletedFor.current === scrollKey) return

    const frame = window.requestAnimationFrame(() => {
      const exactTarget = document.getElementById(targetDate)
      const target = exactTarget ?? document.querySelector<HTMLElement>("[data-moment-date]")
      if (!target) return
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const page = await retryRead(() => listPosts({ limit: 20, cursor: nextCursor }))
          ...
          onClick: () => {
            void (async () => {
              try {
                ...
                toast.add({ type: "error", description: "撤销失败，请稍后重试。" })
              }
            })()
          },
        },
  ...
  return (
    <MomentsShell actions={actions}>
      {isAdmin ? (
        <div className="mb-12">
          <Composer ref={composerRef} getToken={getToken} onCreated={handleCreated} />
        </div>
      ) : null}
