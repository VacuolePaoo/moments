...
export function MomentDateDetail({ date }: { date: string }) {
  ...
  async function handleDelete(post: MomentPost) {
    ...
    setDetail((current) => current
      ? { ...current, items: current.items.filter((item) => item.id !== post.id) }
      : current)
            ...
            void (async () => {
              try {
                ...
                if (wasLast) {
                  ...
                  setDetail((current) => current
                    ? { ...current, items: mergePosts(current.items, [restored]) }
                    : current)
                }
              } catch {
  ...
  const actions = (
    <Link href={`/#${date}`} className={buttonVariants({ variant: "link", className: "w-fit px-0" })}>
      返回
    </Link>
  )
...
function DateNavigation({ detail }: { detail: DateDetail }) {
  return (
    <ButtonGroup aria-label="日期翻页" className="shrink-0">
      <Tooltip>
        <TooltipTrigger
          render={detail.navigation.newerDate
            ? (
                <Link
      ...
      <Tooltip>
        <TooltipTrigger
          render={detail.navigation.olderDate
            ? (
                <Link
