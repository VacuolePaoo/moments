...
export function listPosts(options: {
  ...
  return request<PostList>(`/api/v1/posts?${query.toString()}`, { signal: options.signal })
}

export function getDateDetail(date: string, signal?: AbortSignal): Promise<DateDetail> {
  return request<DateDetail>(`/api/v1/dates/${encodeURIComponent(date)}`, { signal })
}

export function getAuthStatus(token: string, signal?: AbortSignal): Promise<AuthStatus> {
  return request<AuthStatus>("/api/v1/auth/me", { token, signal })
}

export function createPost(content: string, token: string): Promise<MomentPost> {
...
export function updatePost(id: string, content: string, token: string): Promise<MomentPost> {
...
export function restorePost(id: string, token: string): Promise<MomentPost> {
  return request<MomentPost>(`/api/v1/posts/${encodeURIComponent(id)}/restore`, {
    method: "POST",
    token,
  })
}
