...
export const CalendarHeatmapBody = ({
  ...
  const weekdayLabelWidth = hideWeekdayLabels ? 0 : 24;
  ...
  return (
    <div
      ...
      className={cn("flex max-w-full flex-col gap-6 py-4", className)}
      {...props}
    >
      {rowData.map(
        ({ yearRow, width, height, monthLabels, yearTotalCount }) => (
            ...
            <div className="flex min-w-0 items-start">
              {!hideWeekdayLabels ? (
                <svg
                  ...
                  viewBox={`0 0 ${weekdayLabelWidth} ${height + strokePadding * 2}`}
              ...
              <ScrollArea className="min-w-0 flex-1 pb-3">
                <svg
                  role="img"
                  aria-label={(
                  ...
                  height={height + strokePadding * 2}
                  viewBox={`0 0 ${width + strokePadding * 2} ${height + strokePadding * 2}`}
                  width={width + strokePadding * 2}
                >
                  <g
                    transform={`translate(${strokePadding}, ${strokePadding})`}
                  >
                    {!hideMonthLabels ? (
                      <g
                        className={cn("fill-current font-mono", labelClassName)}
                      >
